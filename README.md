...
這是一個純 Python 實作的解析器，專門用於解析 Exptech 定義的 **RTS Binary Data Format v1**。

本專案實作邏輯嚴格參照 [rts.hexpat](rts.hexpat) (ImHex Pattern) 的定義。
...
- **數值轉換**: 自動處理 `VarInt`、`Time40` (5 bytes timestamp) 與 `Intensity` 的數值轉換與正規化。
...
- **IntensityAlert**: 單一位元組，包含震度數值與 Alert 旗標。
